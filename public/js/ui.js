...
const UI = {
  ...
  _bindMainMenu() {
    ...
    this.el["btn-learning-games"].addEventListener("click", () => {
      this.hideAllScreens();
      this.show("learning-games-screen");
      CardEffects.shuffleIn(this.el["game-tile-grid"]); // premium: card-shuffle entrance
    });

    // HUD buttons (in-game)
    this.el["btn-open-map"].addEventListener("click", () => {
      this.app.pause();
      this.hideAllScreens();
      this.show("worldmap-screen");
      CardEffects.spreadOut(this.el["world-tile-grid"]); // premium: card-spread entrance
    });
